/*
  # Populate Sample Data for Technical Service Management System

  1. Sample Data Creation
    - `users` - System users with different roles (admin, manager, supervisor, technician, operator)
    - `clients` - Individual and company clients with realistic contact information
    - `service_requests` - Technical service requests with various statuses and priorities
    - `notifications` - System notifications for users
    - `audit_logs` - Sample audit trail entries

  2. Data Relationships
    - Service requests assigned to technicians
    - Requests linked to clients
    - Approval workflow with managers/supervisors
    - Realistic timestamps and status progression

  3. Technical Scenarios
    - Common IT issues (network, hardware, software)
    - Industrial equipment maintenance
    - Emergency and routine service calls
...
SELECT setval('clients_id_seq', (SELECT MAX(EXTRACT(EPOCH FROM created_at))::bigint FROM clients), true);
SELECT setval('service_requests_id_seq', (SELECT MAX(EXTRACT(EPOCH FROM created_at))::bigint FROM service_requests), true);
SELECT setval('notifications_id_seq', (SELECT MAX(EXTRACT(EPOCH FROM created_at))::bigint FROM notifications), true);
SELECT setval('audit_logs_id_seq', (SELECT MAX(EXTRACT(EPOCH FROM timestamp))::bigint FROM audit_logs), true);