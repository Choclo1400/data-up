/*
  # Populate database with realistic technical service data

  This migration adds sample data for a technical service management system including:

  1. **Users (13 records)**
     - 1 Admin, 2 Managers, 2 Supervisors, 6 Technicians, 2 Operators
     - Realistic names, emails, roles, and login timestamps
     - Two-factor authentication enabled for key personnel

  2. **Clients (10 records)**
     - 3 Individual clients and 7 company clients
     - Complete contact information and addresses
     - Mix of active business relationships

  3. **Service Requests (12 records)**
     - Various service types: network, hardware, software, security, emergency
     - Different statuses: completed, in-progress, approved, pending, cancelled
     - Realistic costs, materials, and technical details
     - Proper technician assignments and approval workflows

  4. **Notifications (7 records)**
     - Task assignments, emergency alerts, approval reminders
     - Different notification types and read statuses

  5. **Audit Logs (10 records)**
     - User activities, system changes, data exports
     - Complete audit trail with IP addresses and user agents

  All data is interconnected with proper foreign key relationships and realistic business scenarios.
*/

-- Insert sample users (system personnel)
